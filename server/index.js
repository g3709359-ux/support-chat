...
io.on('connection', (socket) => {
  ...
  if (role === 'agent') {
    ...
    socket.on('agent_message', ({ visitorId, text }) => {
      const msg = { id: uuidv4(), from: 'agent', text, ts: Date.now() };
  ...
  socket.on('visitor_message', ({ text }) => {
    const msg = { id: uuidv4(), from: 'visitor', text, ts: Date.now() };
    conversations[visitorId].messages.push(msg);
